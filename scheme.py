...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        operand = scheme_eval(first, env)
        check_procedure(operand)
        if not isinstance(operand, MacroProcedure):
            args = rest.map(lambda x: scheme_eval(x, env))
            return scheme_apply(operand, args, env)
        else:
            return scheme_eval(operand.apply_macro(rest, env), env)
        # if isinstance(operands, MacroProcedure):
        #     return scheme_eval(operand.apply_macro(rest, env), env)
        # operators = rest.map(lambda x: scheme_eval(x, env))
        # return scheme_apply(operand, operators, env)
        # END PROBLEM 4
